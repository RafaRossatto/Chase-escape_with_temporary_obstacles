...
input_file = output_dir / f"derivadas_rg_SRE_{sre}.csv"
figures_dir = output_dir / "derivadas_rg_plots"
...
def find_extreme_point(df_frac, column='drg_dp'):
...
def find_inflexion_points(df_frac):
    ...
    for i in range(len(df_frac) - 1):
        d2_i = df_frac.iloc[i]['d2rg_dp2']
        d2_next = df_frac.iloc[i + 1]['d2rg_dp2']
...
def plot_derivatives_for_frac(df_frac, frac):
    ...
    d1 = df_frac['drg_dp'].values
    d1_err = df_frac['drg_dp_error'].values
    
    # Curva principal
    ax1.plot(prob, d1, 'o-', color='navy', linewidth=2, markersize=6, label='d⟨Rg⟩/dp')
    ...
    # Ponto extremo
    extreme = find_extreme_point(df_frac, 'drg_dp')
    if extreme:
        ...
        # Anotação
        d1_valid = d1[~np.isnan(d1)]
        if len(d1_valid) > 1:
            offset_y = 0.05 * (np.max(d1_valid) - np.min(d1_valid))
        else:
            offset_y = 0.1
    ...
    ax1.set_ylabel('d⟨Rg⟩/dp', fontsize=14)
    ax1.set_title(f'Derivada Primeira - Fração {frac}%, SRE = {sre}', fontsize=14, fontweight='bold')
    ax1.grid(True, alpha=0.3, linestyle='--')
    ax1.legend(loc='best', fontsize=10)
    ax1.set_ylim(-28, 1.00)
    
    # ===== GRÁFICO 2: DERIVADA SEGUNDA =====
    d2 = df_frac['d2rg_dp2'].values
    d2_err = df_frac['d2rg_dp2_error'].values
    
    # Curva principal
    ax2.plot(prob, d2, 's-', color='darkgreen', linewidth=2, markersize=6, label='d²⟨Rg⟩/dp²')
    ...
    extreme2 = find_extreme_point(df_frac, 'd2rg_dp2')
    ...
    #             markeredgewidth=1.5, label='Extremo da d²⟨Rg⟩/dp²')
    
    ax2.set_xlabel('Probabilidade', fontsize=14)
    ax2.set_ylabel('d²⟨Rg⟩/dp²', fontsize=14)
    ...
    ax2.set_ylim(-400, 390)
    
    plt.tight_layout()
    
    # Salva a figura
    output_file = figures_dir / f"derivadas_rg_frac_{frac}_SRE_{sre}.png"
...
def plot_all_derivatives_together(df):
    ...
    output_file = figures_dir / f"derivadas_rg_todas_frac_SRE_{sre}.png"
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    plt.close(fig)
...
def create_inflexion_summary(df):
    ...
    if all_inflexions:
        ...
        output_file = figures_dir / f"inflexoes_rg_SRE_{sre}.csv"
...
def create_extreme_summary(df):
    """Cria uma tabela resumo com os pontos extremos da derivada primeira de cada fração"""
    summary = []
    ...
    for frac in sorted(df['frac'].unique()):
        ...
        if extreme:
            summary.append({
                'frac': frac,
                'p_extremo': extreme['prob'],
                'drg_dp_extremo': extreme['value'],
                'erro_extremo': extreme['error'],
                'tipo': 'máximo' if extreme['is_max'] else 'mínimo'
            })
    
    if summary:
        df_summary = pd.DataFrame(summary)
        output_file = figures_dir / f"extremos_rg_SRE_{sre}.csv"
        df_summary.to_csv(output_file, index=False, float_format='%.6f')
        
        print("\n" + "="*60)
        print("PONTOS EXTREMOS DA DERIVADA PRIMEIRA")
        print("="*60)
        print(df_summary.to_string(index=False))
        print(f"\n✓ Tabela de extremos salva: {output_file}")
...
if __name__ == "__main__":
    print("="*60)
    print("PLOTANDO DERIVADAS PRIMEIRA E SEGUNDA DO RAIO DE GIRO (Rg)")
    print(f"SRE = {sre}")
    print("="*60)
    ...
    if 'd2rg_dp2' not in df.columns:
        print("\n⚠ ERRO: Coluna 'd2rg_dp2' não encontrada!")
    ...
    # Cria tabela resumo de extremos
    create_extreme_summary(df)
